...
public abstract class Upgrade2 {
    ...
    public static List<Upgrade2> upgradeList = new ArrayList<>();                  //Erstellen einer Liste die bei der Erstellung eines neuen Upgrades dieses gleich in sich hinzufÃ¼gt
    private ShopItem2 shopItem;
    ...
    public Upgrade2(int clickpower, int amount, int cost, String name, String url) {
        ...
        shopItem = new ShopItem2(new Image(url), this);                     //Pfad des Icons eines Upgrades
    }
    ...
    protected abstract void setCash(BigInteger subtract);


    protected abstract BigInteger getCash();
    ...
    public void checkIfMoney() {
        if (getCash().longValue() >= cost)
            ...
            System.out.println("WE DONÂ´T HAVE THE CAPACITIES");                    //Wenn wir nicht genug WÃ¤hrung haben wird eine Fehlermeldung aufgefÃ¼hrt.
    }
