package at.acfhcampus.stud.team_garbagecan.Upgrades_4;
...
public abstract class Upgrade4 {
    ...
    public static List<Upgrade4> upgradeList = new ArrayList<>();                  //Erstellen einer Liste die bei der Erstellung eines neuen Upgrades dieses gleich in sich hinzufügt
    private ShopItem4 shopItem;

    public Upgrade4(int income, int amount, int cost, String name, String url) {    //Ganz normaler Konstruktor für die Upgrades
        this.income = income;
        this.amount = amount;
        this.cost = cost;
        this.name = name;
        upgradeList.add(this);                                                      //Fügt das erzeugte Upgrade in die erstellte Liste hinzu
        shopItem = new ShopItem4(new Image(url), this);                     //Pfad des Icons eines Upgrades
    }
